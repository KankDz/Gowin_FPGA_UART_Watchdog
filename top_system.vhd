...
entity top_system is
    port (
        -- Tin hieu he thong co ban
        clk               : in  std_logic;
        -- Da xoa port rst de noi cung ben trong

        -- Giao tiep UART voi PC (Cam cap USB)
        rx_i              : in  std_logic;
        tx_o              : out std_logic;

        -- Giao tiep Ngoai vi Vat ly tren Board Kiwi 1P5
        btn_kick_i        : in  std_logic; -- Nut S1 (WDI)
        btn_en_i          : in  std_logic; -- Nut S2 (EN)
        
        wdo_o             : out std_logic; -- LED D3 (Bao loi Watchdog)
        enout_o           : out std_logic  -- LED D4 (Bao Enable Out)
    );
end top_system;
...
architecture rtl of top_system is
    ...
    -- DAY NOI CUNG RESET
    signal rst_tie_low  : std_logic := '0'; -- Danh cho khoi Active-High
    signal rst_tie_high : std_logic := '1'; -- Danh cho khoi Active-Low
...
begin
    ...
    -- ==========================================
    -- 4. RAP KHOI UART_ENGINE
    -- ==========================================
    u_engine : uart_engine port map (
        clk        => clk,
        rst        => rst_tie_low,   -- [FIX] Reset Tich cuc CAO -> Noi cung 0
        empty_o    => w_rx_empty,
        rdata_o    => w_rx_data,
        rd_i       => w_rx_read,
        full_o     => w_tx_full,
    ...
    u_regfile : regfile port map (
        clk               => clk,
        rst_n             => rst_tie_high, -- [FIX] Reset Tich cuc THAP -> Noi cung 1
        wr_en             => bus_wr_en,
        rd_en             => bus_rd_en,
        addr              => bus_addr,
        wdata             => bus_wdata,
